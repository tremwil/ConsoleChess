...
public:
	...
	{
		IVec2 start = IVec2(CLAMP(layerPos.x, 0, w - 1), CLAMP(layerPos.y, 0, h - 1));
		...
		for (int x = start.x; x < end.x; x++)
		{
			...
			{
				IVec2 v = IVec2(x, y);
				if (sprite[v - start] != alpha) buffer[y * h + x] = sprite[v - start];
			}
		}
	}
	...
	void overlay(const Layer &other, byte alpha)
	{
		...
		for (int x = start.x; x < end.x; x++)
		{
			...
			{
				IVec2 v = IVec2(x, y);
				if (other[v - other.pos] != alpha) this->operator[](v - pos) = other[v - pos];
			}
		}
	}
